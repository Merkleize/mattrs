It's often been claimed that "covenants don't solve the L2 exit problem" because it's always not economical to exit a layer 2 system for a user whose balance is too low to cover the fees.
...
As long as the interested parties in a smart contract have a way of disproving fraudulent claims - and a protocol that punishes any fraud attempt, then frauds (and fraud proofs) are never expected to end up on chain.
...
The trade-offs of using protocols based on fraud proofs (optimistic protocols) in comparison to a full on-chain validation of the contract transition are:
...
**Remark**. While [MATT](https://merkle.fun) admits a generic fraud proof protocol for arbitrary computations (that scales logarithmically on its computational cost), there are many special cases for which the fraud proof protocol is a lot simpler. Fraud proofs on statements about the leaves of a Merkle tree (for example "if I change the $i$-th leaf of the Merkle tree with root $R$, the root of the new Merkle tree is $R'$") have fairly straightforward fraud proof protocols).
...
A set $S$ of users wants to delegate to Ingrid the right to withdraw their "aggregate balance". Each of them signs a delegation message with their pubkey in the pool saying "I authorize Ingrid to withdraw my balance".
...
For simplicity, we said that Ingrid "custodially" helps $S$ above, but one can build on top of this scheme: for example, the delegation could already specify some other UTXO(s) where the money has to go once the challenge period is over (maybe a new pool with different operators agreed upon by the users in $S$).

OP_CHECKTEMPLATEVERIFY or OP_TXHASH would provide an efficient way to pre-program the future of the coins after the aggregated withdrawal.

### Constant withdrawal cost? (open problem)

In the above, the index of each user who is withdrawing is part of the initial aggregate withdrawal message. While that doesn't require too many bytes, it still makes the transaction size increase linearly with the number of users withdrawing.
...
It would be great to find a protocol with constant cost independent of $n$ or $|S|$.
