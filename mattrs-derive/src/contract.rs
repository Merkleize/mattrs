...
/// `bob_reveal_left` → `BobRevealLeft`, for clause enum variants.
...
fn codegen(def: ContractDef) -> TokenStream2 {
    let name = &def.name;
    let identity_ident = format_ident!("__{}ContractIdentity", name);
    let params_ty = &def.params_ty;
    let handle_ident = format_ident!("{}Handle", name);
    let clause_enum_ident = format_ident!("{}Clause", name);
    ...
    // The typed clause enum: one variant per declared clause, so spend
    // classification can match on checked identifiers instead of name strings.
    let clause_names: Vec<String> = def.clauses.iter().map(|c| c.name.to_string()).collect();
    let clause_variants: Vec<Ident> = def
        .clauses
        .iter()
        .map(|c| Ident::new(&camel_case(&c.name.to_string()), c.name.span()))
        .collect();

    // The ctx-dependent pieces of the contract struct and its `new`.
    let (ctx_struct_field, ctx_new_param, ctx_init, ctx_self_field, ctx_accessor) = match ctx_ty {
        Some(c) => (
            quote! { ctx: ::std::sync::Arc<#c>, },
            quote! { , ctx: #c },
            quote! { let ctx = ::std::sync::Arc::new(ctx); },
            quote! { ctx, },
            quote! {
                /// The immutable construction context captured by this contract.
    ...
    quote! {
        ...
        struct #identity_ident;

        /// This contract's clauses, typed. Generated by `contract!`: match on
        /// this (via the handle's `spent_clause`) instead of clause-name
        /// strings, so a renamed clause breaks the build, not the runtime.
        #[derive(
            ::core::clone::Clone,
            ::core::marker::Copy,
            ::core::cmp::PartialEq,
        ...
        pub enum #clause_enum_ident {
            #(#clause_variants,)*
        }

        impl #clause_enum_ident {
            /// The clause's name in the contract's taptree.
            pub fn name(self) -> &'static str {
                match self {
                    #(Self::#clause_variants => #clause_names,)*
                }
            }

            /// The typed clause with this taptree name, if it is one of this
            /// contract's.
            pub fn from_name(name: &str) -> ::core::option::Option<Self> {
                match name {
                    #(#clause_names => ::core::option::Option::Some(Self::#clause_variants),)*
        ...
        impl #name {
            ...
            pub fn new(
                ...
                #(#clause_locals)*
                #tree_init
                let contract = <#contract_ty>::new_with_identity::<#identity_ident>(
                    ::core::stringify!(#name),
                    internal_key,
                    &params,
                    tree,
            ...
            pub fn params(&self) -> #params_ty {
                <#contract_ty as ::mattrs::contracts::ErasedContract>::params_any(&self.contract)
                    .and_then(|p| p.downcast_ref::<#params_ty>())
                    .expect("a generated contract carries its typed params")
                    .clone()
            }

            /// The underlying contract primitive, for read-only inspection.
            pub fn contract(&self) -> &#contract_ty {
                &self.contract
            }
        ...
        impl #handle_ident {
            ...
            /// The clause a recorded spend of this instance went through
            /// (`None` while unspent, or when the spend was not decoded back
            /// to a clause).
            pub fn spent_clause(&self) -> ::core::option::Option<#clause_enum_ident> {
                self.0
                    .clause_name()
                    .and_then(|n| #clause_enum_ident::from_name(&n))
            }
        ...
        impl ::core::convert::TryFrom<::mattrs::manager::InstanceHandle> for #handle_ident {
            ...
            fn try_from(
                ...
                if handle.contract_type_id() == ::core::any::TypeId::of::<#identity_ident>() {
                    ::core::result::Result::Ok(#handle_ident(handle))
                } else {
